// Server-only module: imported solely by API route handlers. The USDA key below
// uses a non-NEXT_PUBLIC env var so it is never included in the browser bundle.
import { Product, Additive, NutritionData } from '@/types';
import { calculateScore } from '@/lib/score';

// USDA FoodData Central API configuration (server-side only — key never reaches the browser)
...
export async function searchUSDAFoods(query: string, pageSize: number = 10): Promise<Product[]> {
  ...
  try {
    // Foundation foods (raw fruits, vegetables, meats) + SR Legacy (standard reference)
    ...
    // Prioritize raw/whole foods, filter out branded/processed
    foods = foods
      .filter((food: any) => {
